window.carouselConfig = {
  works: [
    {
      img: 'https://i.imgur.com/1Q9Z1ZB.jpg',
      title: '《大方廣圓覺修多羅了義經講義附親聞記》',
      subtitle: '原刊書名：大方廣圓覺修多羅了義經講義附親聞記',
      pubinfo: '出版資訊：台北：財團法人佛陀教育基金會，2020年出版',
      date: '出版/發行時間：2020年'
    },
    {
      img: 'https://i.imgur.com/6Qw1QwB.jpg',
      title: '《金剛般若波羅蜜經講義》',
      subtitle: '原刊書名：《金剛般若波羅蜜經講義》',
      pubinfo: '原刊出版資訊：大藏經補編 第7冊 No.23 金剛般若波羅蜜經講義',
      date: '出版/發行時間：2024年11月'
    },
    {
      img: 'https://i.imgur.com/8Qw1QwC.jpg',
      title: '《大乘止觀述記》',
      subtitle: '原刊書名：《大乘止觀述記》',
      pubinfo: '出版資訊：台北：財團法人佛陀教育基金會，2010年修訂版',
      date: '出版/發行時間：2010年'
    }
  ],
  research: [
    {
      img: 'https://i.imgur.com/6Qw1QwB.jpg',
      title: '《金剛般若波羅蜜經講義》研究',
      subtitle: '研究主題：江味農對《金剛經》的現代詮釋',
      pubinfo: '學術論文、期刊、專書等',
      date: '近年發表：2022-2024'
    },
    {
      img: 'https://i.imgur.com/1Q9Z1ZB.jpg',
      title: '《圓覺經親聞記》研究',
      subtitle: '研究主題：江味農與圓覺思想',
      pubinfo: '佛學會議論文、碩博士論文',
      date: '近年發表：2020-2023'
    }
  ],
  citation: [
    {
      img: 'https://i.imgur.com/8Qw1QwC.jpg',
      title: '《大乘止觀述記》弘化引用',
      subtitle: '引用場合：現代佛教講座、禪修營',
      pubinfo: '引用單位：法鼓山、各大佛學院',
      date: '2021-2024'
    },
    {
      img: 'https://i.imgur.com/6Qw1QwB.jpg',
      title: '《金剛般若波羅蜜經講義》弘化引用',
      subtitle: '引用場合：佛教經典導讀、社會教育',
      pubinfo: '引用單位：佛光山、淨空法師弘法團體',
      date: '2020-2024'
    }
  ],
  journal: [
    {
      img: 'https://i.imgur.com/1Q9Z1ZB.jpg',
      title: '江味農居士佛學思想初探',
      subtitle: '作者：李明哲',
      pubinfo: '發表於《現代佛學》期刊第12期',
      date: '2022年6月'
    },
    {
      img: 'https://i.imgur.com/6Qw1QwB.jpg',
      title: '江味農與近代佛教刊物',
      subtitle: '作者：王小華',
      pubinfo: '發表於《佛教文化》雜誌第45期',
      date: '2021年12月'
    },
    {
      img: 'https://i.imgur.com/8Qw1QwC.jpg',
      title: '江味農居士的經典詮釋方法',
      subtitle: '作者：陳志遠',
      pubinfo: '發表於《宗教學研究》期刊第30卷',
      date: '2023年3月'
    },
    {
      img: 'https://i.imgur.com/1Q9Z1ZB.jpg',
      title: '江味農與民國佛教期刊',
      subtitle: '作者：林慧君',
      pubinfo: '發表於《民國佛教研究》2020年第2期',
      date: '2020年8月'
    }
  ]
}; 